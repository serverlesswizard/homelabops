...
options:
  ...
  authMode:
    description: The association control method for the SSID ('open', 'open-enhanced',
      'psk', 'open-with-radius', 'open-enhanced-with-radius', 'open-with-nac', '8021x-meraki',
      '8021x-nac', '8021x-radius', '8021x-google', '8021x-entra', '8021x-localradius',
      'ipsk-with-radius', 'ipsk-without-radius', 'ipsk-with-nac' or 'ipsk-with-radius-easy-psk').
    type: str
  availabilityTags:
  ...
  ipAssignmentMode:
    description: The client IP assignment mode ('NAT mode', 'Bridge mode', 'Layer
      3 roaming', 'Ethernet over GRE', 'Layer 3 roaming with a concentrator', 'VPN'
      or 'Campus Gateway').
    type: str
  lanIsolationEnabled:
  ...
  ldap:
    ...
    type: dict
  localAuthFallback:
    description: The current configuration for Local Authentication Fallback. Enables
      the Access Point (AP) to store client authentication data for a specified duration
      that can be adjusted as needed.
    suboptions:
      cacheTimeout:
        description: The duration (in seconds) for which auths are cached. The timeout
          is measured from the user's most recent non-cached authentication to the
          network. Between 3600 (1 hour) and 86400 (1 day).
        type: int
      enabled:
        description: If true, MR devices will cache authentication credentials for
          EAP-TLS or for MAC based authentication.
        type: bool
      serverCaCertificate:
        description: The Server CA Certificate used to sign the client certificate.
        suboptions:
          contents:
            description: The contents of the Server CA Certificate. Must be in PEM
              or DER format.
            type: str
        type: dict
    type: dict
  ...
  radiusAccountingStartDelay:
    description: The delay (in seconds) before sending the first RADIUS accounting
      start message. Must be between 0 and 60 seconds.
    type: int
  ...
  splashPage:
    description: The type of splash page for the SSID ('None', 'Click-through splash
      ...
      'Google OAuth', 'Microsoft Entra ID', 'Sponsored guest', 'Cisco ISE' or 'Google
